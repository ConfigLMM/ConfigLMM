    ...
    module LMM
        class Answer < Framework::Plugin
            ...
            def actionAnswerDeploy(id, target, activeState, context, options)
                self.withConnection(target['Location'], target) do |connection|
                    Linux.withConnection(connection) do |linuxConnection|
                        ...
                        linuxConnection.createServiceUser(USER, HOME_DIR, 'Apache Answer', options)
                        ...
                        if !linuxConnection.filePresent?(HOME_DIR + '/data/conf/config.yaml')
                            ...
                            linuxConnection.withUserShell(USER) do |shell|
                                ...
                                installEnv = "AUTO_INSTALL=true SITE_ADDR=127.0.0.1:#{PORT} INSTALL_PORT=#{PORT+1} "
                                ...
                                shell.exec(installEnv + INSTALL_PATH + "answer init --data-path #{HOME_DIR}/data", false, { **options, hide: true })

                                prompt.say("Answer Administrator #{target['Admin']['EMail']} password: #{adminPassword}", :color => :magenta)
                            end

                            linuxConnection.fileReplace(HOME_DIR + '/data/conf/config.yaml', 'show: true', 'show: false', options)
                        end
                        ...
                        linuxConnection.ensureServiceAutoStart('answer.service', options)
                        linuxConnection.startService('answer.service', options)
                    end
                end
            end
            ...
            def configureDatabase(target, linuxConnection, options)
                ...
                target['Database']['Name'] = USER unless target['Database']['Name']
                target['Database']['User'] = USER unless target['Database']['User']
            ...
            def cleanup(configs, state, context, options)
                cleanupType(:Answer, configs, state, context, options) do |item, id, state, context, options, connection|
                    Linux.withConnection(connection) do |linuxConnection|
                        linuxConnection.stopService('answer.service', options)
                        linuxConnection.disableService('answer.service', options)
